...
public:
	...
	const std::vector<int> special_ids = {151644,151645};
...
//		 }
		for (const simdjson::dom::key_value_pair& key_value : object) {
            ...
            int64_t value;
            key_value.value.get<int64_t >().get(value);
			encoder.emplace(key, value);
			decoder.emplace(value, key);
		}
	...
	std::vector<int> encode_template(const std::string &text) {
		std::regex B = std::regex(join(special_tokens_regex,'|'));

		auto words_begin = std::sregex_iterator(text.begin(), text.end(), B);
		auto words_end = std::sregex_iterator();

		std::vector<std::size_t> split_pos = {0};
		for (std::sregex_iterator i = words_begin; i != words_end; ++i) {
			std::smatch match = *i;
			std::size_t position = match.position();
			std::size_t token_len = match.str().length();
			split_pos.push_back(position);
			split_pos.push_back(position+token_len);
		}
		split_pos.push_back(text.length());

		std::vector<int> input_ids;
		for (std::size_t i = 1; i < split_pos.size(); i++) {
			std::string sub = text.substr(split_pos[i-1],split_pos[i]-split_pos[i-1]);
			if (sub.length() > 0) {
				auto ids = encode(sub);
				for (int j = 0; j < special_tokens.size(); j++) {
					if (sub == special_tokens[j]) {
						ids = {special_ids[j]};
						break;
					}
				}
	...
	std::string decode_skip(int token_id) {
		for (auto special_id : special_ids) {
	...
	std::string decode(const std::vector<long long> &token_ids) {
